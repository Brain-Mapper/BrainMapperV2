        ...
        if get_selected():
            ...
            if ret == QtGui.QMessageBox.Apply:
                ...
                if nifti_opt.isChecked():
                    # return an addition of all the file as one (and only one) nifti file
                    # folder_path = str(QFileDialog.getExistingDirectory())
                    # image_recreation_from_list(folder_path, selected)
                    path = QFileDialog.getSaveFileName(None, "Save the nifti file", )
                    result_matrix = calculations.addition_opperation(selected_images_collections)
