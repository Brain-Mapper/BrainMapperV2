                    ...
                    for sub_item in item_list:
                        if not sub_item.startswith('.'):
                            ...
                            if extension in ['nii', 'nii.gz']:
                                actual_set.collection_dict[item].add(
                                    NifImage.from_file(os.path.join(item_path, sub_item)))
                            else:
                                if extension in ["xls", "xlsx"]:
                                    actual_set.collection_dict[item].add(
                                        image.ExcelImage(os.path.join(item_path, sub_item)))
                                elif extension in ["csv"]:
                                    actual_set.collection_dict[item].add(
                                        image.CSVImage(os.path.join(item_path, sub_item)))
